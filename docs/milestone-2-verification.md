...
> The historical finite-branch verification below is retained as a record of
> the earlier milestone. The current active-call contract is open-ended:
> participant turns remain non-terminal until the explicit **End call safely**
> action, and the current proof is recorded by the focused lifecycle checks and
> `scripts/smoke_test.py`.

## Current open-ended call contract (September 20, 2026)

- The opening and every active caller response are generated by local Ollama
  when `DIALOGUE_PROVIDER=ollama`; authored dialogue is not substituted after
  an Ollama failure. A failed response preserves the session and exposes an
  explicit retry action.
- `fast_safety_policy` classifies every active participant turn. The rolling
  deterministic engine updates strategy, risk, tactics, evidence, and timeline
  without setting a terminal outcome.
- Questions, skepticism, discomfort, refusal, apparent compliance, and “I am
  hanging up” remain active turns. Five or more ordinary turns are supported.
- `POST /api/sessions/{id}/end` is the normal terminal trigger. It returns the
  deterministic training debrief immediately; optional Nemotron evaluation is
  scheduled only afterward and is non-blocking.
- The required judge proof is the bank-fraud voice session with five distinct
  Ollama turns, an active session after turn five, and a debrief only after the
  explicit end action.
